...
${book_id}
${book_name}        La RobotFramework
*** Test Cases ***


Exploring Dictinoary
    [Tags]      API
    &{data}=    Create Dictionary      name=Rajabhishek     course=robot       designation=PAT
    log     ${data}
    Dictionary Should Contain Key     ${data}     name
    log     ${data}[name]
    ${url}=     Get From Dictionary     ${data}     name
    log   ${url}


Add Book into Library DataBase
        [Tags]      API
      &{req_body}=  Create Dictionary    name=${book_name}        isbn=947303     aisle=324534        author=rajabhishek
